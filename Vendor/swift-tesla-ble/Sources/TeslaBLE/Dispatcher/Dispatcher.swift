...
actor Dispatcher {
    ...
    private func negotiateOnce(
        ...
        // Surface protocol-layer faults (e.g. key not on whitelist / not paired)
        // BEFORE the signature guard below, which would otherwise mask the real
        // reason as the opaque "missing signature data". Mirrors the send() path.
        if response.hasSignedMessageStatus {
            let status = response.signedMessageStatus
            if status.signedMessageFault != .rrorNone {
                throw Error.unexpectedResponse(
                    "negotiate fault on domain \(domain): \(String(describing: status.signedMessageFault))",
                )
            }
        }
