...
public actor TeslaVehicleClient {
    ...
    private var backgroundTransport: BackgroundBLETransport?
    ...
    /// Brings the session up over a peripheral the caller has ALREADY connected
    /// with its own `CBCentralManager` — the background walk-up path.
    ///
    /// Performs no scanning: the caller owns discovery, the standing pending
    /// connection, and the central's state restoration. We only run GATT
    /// discovery + the signed handshake over the supplied peripheral.
    ///
    /// - Parameters:
    ///   - peripheral: A peripheral already in the `.connected` state.
    ///   - delegateQueue: The serial queue the caller's central was created on,
    ///     so peripheral delegate callbacks stay serialized with it.
    ///   - mode: Whether to run the signed handshake after attaching.
    ///   - timeout: Upper bound for attach + handshake.
    public func connect(
        ...
        guard _state == .disconnected else {
            logger?.log(.warning, category: "client", "connect(using:) called while in state \(_state); ignoring")
            return
        }
        let privateKey = try loadPrivateKey()

        let bg = BackgroundBLETransport(peripheral: peripheral, delegateQueue: delegateQueue, logger: logger)
        backgroundTransport = bg
        updateState(.connecting)
        do {
            try await bg.attach(timeout: Self.seconds(timeout))
        } catch {
            logger?.log(.error, category: "client", "BLE attach failed: \(error)")
            await tearDown()
            throw Self.mapTransportError(error)
        }

        let dispatcher = Dispatcher(transport: bg, logger: logger)
        self.dispatcher = dispatcher
        do {
            try await dispatcher.start()
        } catch {
    ...
    private func tearDown() async {
        ...
        transport = nil
        backgroundTransport?.detach()
        backgroundTransport = nil
